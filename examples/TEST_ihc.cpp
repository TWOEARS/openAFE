...
int main(int argc, char **argv) {
  ...
  std::string outputName = "ihc_out.mat"; 
  ...
  if ( result == 0 ) {
	  ...
	  std::vector<std::shared_ptr<twoCTypeBlock<double> > > lOut = ihcP->getLeftWholeBufferAccessor();
	  std::vector<std::shared_ptr<twoCTypeBlock<double> > > rOut = ihcP->getRightWholeBufferAccessor();
